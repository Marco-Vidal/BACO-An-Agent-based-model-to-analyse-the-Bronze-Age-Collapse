...
[
  ...
  storage-limit ; maximum storage capacity expressed in kcal/year
...
set elevation-dataset gis:load-dataset "BASEMAP.asc"
...
to set-constants
  ...
  ask patches
  [
    set soil-capacity random-normal 1600 300 ; productivity capacity expressed in kg/ha/year
  ]
  repeat 2
...
to go
 ...
 if (not any? oikoi)
  ...
  if sea-Ppl? and calendar = B-raids
  [
    create-seapeoples n-sea-people
    [
    set shape "boat"
...
to calculate-soil-productivity
  ...
  [
    ...
    set soil-fertility initial-soil-fertility + ((initial-soil-fertility / soil-capacity)^ degradationfactor)
    * recoverrate * initial-soil-fertility * (1 - initial-soil-fertility / soil-capacity) - Dh
    * oikoi-per-patch

    if (soil-fertility < 0) [set soil-fertility 0.1]

    set expected-Yield (soil-fertility + recoverrate * soil-fertility * ((soil-fertility / soil-capacity) ^ degradationfactor)
    * (1 - soil-fertility / soil-capacity) - Dh * (count oikoi-here + 1))  * slope-factor

    if (expected-yield < 0) [set expected-yield 0.1]

    set rainfall 1.5 * (1 - exp ( (ln(1 / 3) / 4)  * (item cRAI RAI + 4)))

    set rainfall rainfall * (1 + random-normal 0 rainfallvariability)
  ]
...
to oikoi_harvesting
  ...
  ask oikoi-here
    [
      ...
      [
        set farming-assets 1 / (1 + FAF)
      ]
      [
        set farming-assets (total-residence-continuity / (total-residence-continuity + (Max-farming-assets-improvement - FAF)))
      ]
     ]
  ]
...
set storage-limit storagelimit * 3390000; 3,390,000 kcal = 1 ton of resources
  ...
  [
    ...
    if (oikoi_stocks > storage-limit)
    [set oikoi_surplus (oikoi_stocks - storage-limit)
      set oikoi_stocks storage-limit
    ]
  ]
end
...
to simulate-population-dynamics
  ...
  ask oikoi
  [
    let adjusted-deathrate (( 1 + (oikoi_shortage / oikoi_energy-requirements)) * deathrate)
  ...
  [
    ...
    [
      ...
      let adjusted-birthrate ((energetic-balance / max-e) * Birthrate) * (1 - (P / K))
